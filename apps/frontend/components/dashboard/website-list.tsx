          ...
          {websites.map((website) => (
            ...
            >
              ...
              <div
                ...
                onClick={() => {website.lastChecked == "0" ? null :handleWebsiteClick(website.id)}}
              >
                <div className="flex items-start space-x-3 flex-1 min-w-0">
                 {website.lastChecked == "0" ? <Skeleton className="h-2 w-5" /> :<div className="flex-shrink-0 mt-0.5">{getStatusIcon(website.status)}</div>}  
                  {website.lastChecked == "0"? <Skeleton className="h-10 w-50" />  : <div className="flex-1 min-w-0 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2 min-w-0 flex-1">
                        ...
                        <Button
                          ...
                          asChild
                          onClick={(e) => e.stopPropagation()} // prevent left click
                        >
                    ...
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-1 text-xs text-muted-foreground">
                      ...
                      <span className="hidden lg:flex items-center gap-1.5">
                        ...
                        <span className="font-mono">{new Date(website.lastChecked).toLocaleTimeString()}</span>
                      </span>
                    </div>
                  </div>}
                </div>
                {website.lastChecked == "0" ? <Skeleton className="h-5 w-5" /> : <div className="hidden sm:flex flex-shrink-0 ml-4">{getStatusBadge(website.status)}</div>
           }
                   </div>
            </div>
          ))}
